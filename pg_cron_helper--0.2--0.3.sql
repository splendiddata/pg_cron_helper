    ...
    if current_database() = cron_db_name then
        ...
        begin
            ...
            if v_cron_pattern = '' then
                -- the job definition may forget about this run
                update cron.job_definition
                set cron_pattern = ''
                  , jobid = null
                where pk = v_description_pk;
                -- Unschedule in a week to not have the execution cancelled before it finds its natural end
                sql := $sql$do $exec$
declare
     result_str           text;
     sql                  text := $sql$ || format('''do $sql$begin perform cron.unschedule(job_id => %s); end$sql$''', v_jobid) || $sql$;
begin
...
$sql$;
                perform cron.alter_job( job_id => v_jobid
                                      , schedule => cron._srvr_cron_at_timestamp(now() + interval '1 week')
                                      , command => sql
                                      );
                return;
            end if;
